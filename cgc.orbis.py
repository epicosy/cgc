...
class CGC(BenchmarkHandler):
    ...
    def make(self, context: Context, handler: MakeHandler, write_build_args: str = None,
        ...
        if write_build_args:
            ...
            commands = handler.get_cmake_commands(working_dir=context.root, skip_str="-DPATCHED",
                                                  src_dir=context.source / context.project.modules['src'],
                                                  build_dir=context.build, compiler_trail_path=compiler_trail_path)

            handler.write_cmake_build_args(dest=Path(write_build_args), vuln_files=context.project.vuln_files,
                                           commands=commands, working_dir=context.root)

        return cmd_data

    def build(self, context: Context, handler: BuildHandler, coverage: bool = False, fix_files: List[AnyStr] = None,
        ...
        if backup:
            cmd_data = handler.backup_manifest_files(out_path=Path(backup), source_path=context.source,
                                                     manifest_files=context.project.vuln_files)

        if link:
            cmd_data = handler.cmake_link_executable(source_path=context.source, cmake_path=cmake_source_path,
                                                     build_path=context.build / context.project.name)
        elif inst_files:
            mappings = context.project.map_files(inst_files, replace_ext=('.c', '.i'), skip_ext=[".h"])
            cmake_commands = handler.get_cmake_commands(working_dir=context.root,
            ...
            inst_commands = handler.commands_to_instrumented(mappings=mappings, commands=cmake_commands,
                                                             replace_str=('-save-temps=obj', ''))
            cmd_data = handler.cmake_build_preprocessed(inst_commands=inst_commands, build_path=context.build)
            ...
            cmd_data = self.make(context=context, handler=handler, write_build_args=write_build_args,
                                 compiler_trail_path=compiler_trail_path, replace=replace, save_temps=save_temps)

            cmd_data = handler.cmake_build(target=context.project.name, cwd=str(context.build),
                                           env=self.env)

        return cmd_data, cmake_source_path

    def test(self, context: Context, handler: TestHandler, tests: Oracle, timeout: int, neg_pov: bool = False,
             prefix: str = None, print_ids: bool = False, write_fail: bool = True, only_numbers: bool = False,
             print_class: bool = False, out_file: str = None, **kwargs) -> List[TestOutcome]:
        ...
        for name, test in tests.cases.items():
            ...
            _, outcome = handler.run(context, test, timeout=timeout, script=tests.script, env=self.env, cwd=tests.cwd,
                                     kill=True, args=args, process_outcome=parse_output_to_outcome)
            ...
            if out_file is not None:
                handler.write_result(outcome, out_file=Path(out_file), prefix=Path(prefix) if prefix else None,
